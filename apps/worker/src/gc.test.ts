...
describe("parseGcMessage", () => {
  it("bucket / object_key を取り出す", () => {
    expect(parseGcMessage({ bucket: "wanonwan", object_key: "interviews/x/obj-1" })).toEqual({
      bucket: "wanonwan",
  ...
  it("余分なキーがあっても必要な 2 つを取り出す", () => {
    expect(parseGcMessage({ bucket: "wanonwan", object_key: "k", extra: 1 })).toEqual({
      bucket: "wanonwan",
      objectKey: "k",
    });
  });
  ...
  it.each([
    ...
    ["object_key 欠落", { bucket: "wanonwan" }],
    ["空文字", { bucket: "", object_key: "k" }],
    ["型違い", { bucket: "wanonwan", object_key: 123 }],
