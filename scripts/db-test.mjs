// packages/db/tests/*.test.sql を pgTAP テストとして app_user 接続で実行する。
...
const envFile = process.env.ENV_FILE ?? join(root, "infra", ".env");
const env = loadEnv(envFile);
const db = requireEnv(env, "PG_DATABASE");
const appPassword = requireEnv(env, "APP_DB_PASSWORD");
...
for (const file of files) {
  ...
  try {
    out = execFileSync(
      "docker",
      [
        "compose", "--env-file", envFile, "-f", composeFile, "exec", "-T",
