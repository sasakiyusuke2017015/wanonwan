...
-- フィクスチャ: seed の member1 の回答（サンプル面談アンケート）。member2 は上位ロールなし・無関係。
-- member2 への interviewer ロール付与と担当割り当てはこの tx 内で行い、ROLLBACK で戻す。
BEGIN;
SELECT plan(6);

-- fixture: member2 へ interviewer ロールを付与（admin として）
SET LOCAL app.user_id = 'cb427b54-eaef-47df-916b-626321d23dc9';
INSERT INTO public.user_roles (user_id, role)
  SELECT id, 'interviewer' FROM public.users WHERE email = 'member2@example.com';
...
-- fixture: admin が member1 の回答へ member2 を担当に割り当てる
SET LOCAL app.user_id = 'cb427b54-eaef-47df-916b-626321d23dc9';
UPDATE public.answers a SET interviewer_id =
    (SELECT id FROM public.users WHERE email = 'member2@example.com')
  WHERE a.respondent_id = (SELECT id FROM public.users WHERE email = 'member1@example.com');
SELECT is(
  (SELECT count(*)::int FROM public.answers
   WHERE interviewer_id = (SELECT id FROM public.users WHERE email = 'member2@example.com')),
...
-- fixture: member2 の interviewer ロールを剥奪（admin として）
SET LOCAL app.user_id = 'cb427b54-eaef-47df-916b-626321d23dc9';
DELETE FROM public.user_roles ur USING public.users u
  WHERE ur.user_id = u.id AND u.email = 'member2@example.com' AND ur.role = 'interviewer';
