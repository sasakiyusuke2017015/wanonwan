...
-- member1 を回答者とするサンプル回答を作り、interviewer1 を面談内容閲覧者にする。
-- member2 は無関係（否定テスト）。
...
INSERT INTO public.answers (publication_id, respondent_id, status)
  SELECT p.id, (SELECT id FROM public.users WHERE email = 'member1@example.com'), 200
  ...
  WHERE s.title = 'サンプル面談アンケート'
    AND NOT EXISTS (
      ...
      WHERE a.publication_id = p.id
        AND a.respondent_id = (SELECT id FROM public.users WHERE email = 'member1@example.com')
    );

-- interviewer1 を閲覧者に
INSERT INTO public.answer_viewers (answer_id, user_id)
  SELECT a.id, (SELECT id FROM public.users WHERE email = 'interviewer1@example.com')
