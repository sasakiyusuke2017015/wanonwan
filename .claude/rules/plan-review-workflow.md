...
- **`outputs/` 直下に置けるのは `plans/` / `reviews/` / `README.md` だけ**。検証・受け入れ
  チェック・TODO の類を別ファイルや別ディレクトリに切り出さない（下記「検証結果の置き場所」）。
...
- 検証項目に「**未検証環境を明示する**」のは OK。やらないと決めたものは
  `- [x] macOS: 担当者なしのためスキップ` のように **理由を書いて `[x]` で閉じる**。
  未チェックのまま放置すると `✅ 検証完了` に上がれず、🟢 が永久に残る
...
**検証の真実源は各 Plan の「ステータス」セクションのチェックボックスのみ。**
これと並行する検証ファイル・検証ディレクトリ・受け入れチェックリストを作らない。

**Why:** 二重帳簿は必ず片方だけ更新される。過去に横断チェックリストを別ファイルで持った
結果、そちらだけが埋まって Plan 側に戻らず、**すでに検証済みの機能が 2 ヶ月近く
ダッシュボード上「未検証」のまま残った**。

複数 Plan の残検証をまとめて消化したい（stg でまとめて確認する等）ときは、
**ダッシュボードの「残検証」節を見る**。これは `gen-outputs-readme.mjs` が
全 Plan の未チェック検証項目を集めて自動生成するので、重複なしで横断ビューになる。
足りなければ **生成器を直す**（新しいファイルを作らない）。

実施したら、その場で:

1. 対応 Plan のチェックボックスを `[x]` にし、**日付・実施環境・確認できた事実・
   代替確認や未実施の範囲**を項目本文に書く
2. 全部埋まったら Plan ヘッダを `✅ 検証完了` に上げる
3. `node scripts/gen-outputs-readme.mjs` で再生成する
