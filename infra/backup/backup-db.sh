...
#
# cron 例（毎日 03:10）:
#   10 3 * * * cd /opt/wanonwan && ENV_FILE=infra/.env.prod COMPOSE=infra/docker-compose.prod.yml \
#     BACKUP_DIR=/var/backups/wanonwan sh infra/backup/backup-db.sh >> /var/log/wanonwan-backup.log 2>&1
set -eu

ENV_FILE="${ENV_FILE:-infra/.env.prod}"
COMPOSE="${COMPOSE:-infra/docker-compose.prod.yml}"
BACKUP_DIR="${BACKUP_DIR:-/var/backups/wanonwan}"
RETENTION_DAYS="${RETENTION_DAYS:-14}"

PG_SUPERUSER="$(grep -E '^PG_SUPERUSER=' "$ENV_FILE" | cut -d= -f2)"
PG_DATABASE="$(grep -E '^PG_DATABASE=' "$ENV_FILE" | cut -d= -f2)"
stamp="$(date +%Y%m%d-%H%M%S)"
out="$BACKUP_DIR/wanonwan-$stamp.dump"
...
# 保持期間を過ぎた dump を削除
find "$BACKUP_DIR" -name 'wanonwan-*.dump' -mtime "+${RETENTION_DAYS}" -delete
echo "done. retained $(ls -1 "$BACKUP_DIR"/wanonwan-*.dump 2>/dev/null | wc -l) dump(s)"

# TODO(運用): host 外ストレージへ退避（退避先は笹木さん確定後に有効化）。
#   例: rsync -a "$out" backup-host:/srv/wanonwan-backups/
#   例: aws s3 cp "$out" s3://wanonwan-backups/
