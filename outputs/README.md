...
| 🟠 計画レビュー反映済み（Phase1/2 着手可） | [仕上げ（ページタイトル/公開一覧/ダーク/VRT）](plans/2026-07-05-2015-finishing-touches.md) | UI/UX 改善テーマ5「仕上げ」。**フェーズ分割**: Phase1 ページタイトル / Phase2 公開一覧 UX（polish・即着手可）/ Phase3 ダークモード（epic・三層モデル: semantic トークン反転境界 + design.ts 前景調整 + 114 箇所 repoint・FOUC 対策要）/ Phase4 VRT（epic・reg-suit + storycap + MinIO baseline・4-light→3→4-dark の additive 順序）。epic 2 本は着手時に独立サブ Plan 化 | [計画レビュー](reviews/2026-07-05-2040-finishing-touches-review.md): Phase1/2 APPROVE / Phase3/4 設計指摘反映（代行 planner+architect）| Phase1（ページタイトル）から着手可 / epic は着手時に要判断確定 |
