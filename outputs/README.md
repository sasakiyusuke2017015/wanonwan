...
| 🟦 コードレビュー APPROVE | [API ルートの boilerplate 集約（認証/admin ラッパ + parseBody + service_role + rate-limit）](plans/2026-06-15-1610-api-route-helpers.md) | force-change PR の per-route ガード直書きを関数合成で集約し、認可ガードを構造的に忘れられなくする pure refactor。withActiveUser / parseBody / withServiceRole / checkRateLimit に集約（27 ファイル・純減 117 行） | 計画レビュー 3 本: APPROVE / [コードレビュー(Agent)](reviews/2026-06-15-1710-api-route-helpers-code-review.md): APPROVE | PR 作成（develop 向け）→ 笹木さんマージ承認 |
