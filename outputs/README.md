...
| 🟡 実装完了・PR へ | [apps/web に Vitest 導入 + API ヘルパ unit テスト](plans/2026-06-15-1725-web-vitest-helper-tests.md) | api-route-helpers 残課題。apps/web に Vitest を導入し parseBody / withActiveUser / checkRateLimit / withServiceRole / metadata の unit テスト（5 files / 13 tests）。test-only | レビュー省略（test-only・笹木さん合意） | PR 作成（develop 向け）→ 笹木さんマージ承認 |
