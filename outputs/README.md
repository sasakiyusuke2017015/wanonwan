...
| 🟡 実装中 | [添付ファイル基盤（MinIO + presigned URL）](plans/2026-06-18-1330-attachments-minio.md) | 親 Plan「MinIO/添付」本着手。presigned URL でブラウザが MinIO へ直接 up/down、API は認可+メタのみ。単一 `attachments` 表(polymorphic)+entity_type 別 RLS。Phase1=storage lib+MinIO compose+表/RLS/pgTAP+面談添付 API/UI。Phase2=回答/資料/アバター+`AttachmentsPanel`共通化、Phase3=stg/prod+nginx | [PR #40](https://github.com/sasakiyusuke2017015/waoon/pull/40) Phase1 / Phase2 は #40 に stack | Phase 2 を PR 化（#40 マージ後 develop へ rebase）→ Phase 3 |
| 🟡 実装中 | [AI: 面談メンター提案 + 自由記述の要約/分析（Claude + pgvector）](plans/2026-06-18-1900-ai-pgvector-mentor-summary.md) | 親 Plan「AI 機能」本着手。Phase A=Claude(`@anthropic-ai/sdk`/Sonnet 4.6)で面談要約（`lib/ai`+`POST /answers/[id]/summary`+UI、**ANTHROPIC_API_KEY 未設定で無効=外部送信ゼロ**）。Phase B=pgvector RAG メンター提案（埋め込みプロバイダ要決定・非同期基盤前提） | （未）/ Phase A 実装済（typecheck/lint/test green） | **§3.1 HR データ外部送信の組織承認**が前提。Phase A を PR 化 |
| 🟡 実装中 | [非同期/定期ジョブ基盤（pg_cron + pgmq）](plans/2026-06-18-2030-async-jobs-pgmq-pgcron.md) | 拡張だけ入っていた pgmq/pg_cron に初の実ジョブを載せる。最初=**添付の孤児掃除**（presigned 発行のみで未完了= status 100 の古い行を pg_cron 純 SQL で定期削除）。`app.is_stale_attachment`(純関数・pgTAP) + `app.gc_stale_attachments`(SECURITY DEFINER・cron 30分毎)。Phase2a=pgmq キュー + DELETE enqueue トリガ + 専用 Node worker(`apps/worker`)で MinIO 本体削除（parseGcMessage unit 8・worker は superuser 接続・dev はホスト実行）。Phase2b=Dockerfile.worker + CD + stg/prod compose | [PR #45](https://github.com/sasakiyusuke2017015/waoon/pull/45) Phase1 / Phase2a は新規 PR（typecheck `-r`・test green、pgTAP/worker runtime は CI/笹木さん） | Phase2a を PR 化 → 笹木さん runtime（削除→enqueue→worker 削除）→ Phase2b（prod 配線）|
