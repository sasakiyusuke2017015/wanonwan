...
| 🟣 マージ承認待ち | [編集時の GoTrue 同期 + 初回ログイン後の force-change](plans/2026-06-14-1455-auth-gotrue-sync-force-change.md) | 親 Plan「認証の残」A+B。admin の email/password 変更を GoTrue へ同期（admin client に updateUser 追加）+ 初回 PW の強制変更（force-change を API 層ゲートで enforce、middleware はページ誘導） | [PR #25](https://github.com/sasakiyusuke2017015/waoon/pull/25) / コードレビュー(Codex): NEEDS WORK → [再コードレビュー](reviews/2026-06-15-1030-auth-gotrue-sync-force-change-code-review-v2.md): APPROVE | 笹木さんマージ承認。受け入れ前に B-0 手動確認 |
