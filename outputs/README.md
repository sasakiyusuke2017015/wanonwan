...
| 🔵 計画レビュー待ち | [仕上げ（ページタイトル/公開一覧/ダーク/VRT）](plans/2026-07-05-2015-finishing-touches.md) | UI/UX 改善テーマ5「仕上げ」。**フェーズ分割**: Phase1 ページタイトル（全タブ `waoon` 固定 → title.template + section metadata）/ Phase2 公開一覧 UX（空状態・スケルトン・再試行・締切訴求・回答状態粒度）/ Phase3 ダークモード（epic・トークン 2 系統化 + colorScheme 軸 + 切替 UI + ハードコード色 114 箇所の CSS 変数化）/ Phase4 VRT（epic・reg-suit + 撮影層 + ストレージ + CI・Chromatic 残置整理）。全部含める方針だが epic は着手前に個別設計 | Plan ドラフト（要ユーザー判断: Phase 順序 / ダーク方式 / VRT 路線 / タイトル手段）/ 計画レビュー未 | 笹木さんが計画レビュー → 要判断確定 → Phase1 着手 |
