...
| 🟡 実装中 | [添付ファイル基盤（MinIO + presigned URL）](plans/2026-06-18-1330-attachments-minio.md) | 親 Plan「MinIO/添付」本着手。presigned URL でブラウザが MinIO へ直接 up/down、API は認可+メタのみ。単一 `attachments` 表(polymorphic)+entity_type 別 RLS。Phase1=storage lib+MinIO compose+表/RLS/pgTAP+面談添付 API/UI。Phase2=回答/資料/アバター、Phase3=stg/prod+nginx | （未）/ Phase 1 実装済（lib unit3・pgTAP4・typecheck/lint/test green。実起動は Docker・笹木さん） | Phase 1 を PR 化 → Phase 2 |
