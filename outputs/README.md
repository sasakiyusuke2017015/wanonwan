...
| ⚪ 実装待ち | [緊急度マスタ（surveys / answers 共通）](plans/2026-06-29-1537-urgency-master.md) | 緊急度（高/中/低）を共通マスタ化。PR-1=urgency_levels テーブル+RLS+`/admin/urgencies`+API+seed+pgTAP / PR-2=surveys・answers に urgency_id 配線（SurveyForm・InterviewForm に書き手 UI）。設問マスタ Phase2 から分離 | [計画(Claude)](reviews/2026-06-29-1612-urgency-master-review.md): **APPROVE** | 笹木さん着手承認 → Claude Code が PR-1 から実装 |
