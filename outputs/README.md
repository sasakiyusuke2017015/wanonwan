...
| 🟡 実装中 | [管理一覧のフィルタ/ソート + StatisticPanel](plans/2026-06-18-1115-admin-list-filter-sort-stats.md) | 見た目磨き込み #2。AdminListTable に client-side 検索/ソート（`searchKeys`/`sortable`）を汎用追加 + 集計パネル。ロジックは純関数 `filterRows`/`sortRows`（unit test）。Phase1 users 適用済、Phase2 で surveys/answers + status 別 StatisticPanel | （未）/ Phase 1 実装済（typecheck・lint・test green） | Phase 1 を PR 化 → Phase 2 着手 |
