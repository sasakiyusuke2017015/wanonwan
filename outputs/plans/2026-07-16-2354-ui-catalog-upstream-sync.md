...
| 項目 | 値 |
|---|---|
| 概要 | ui-catalog 上流最新版 (ui.zip) を packages/ui へ選択的マージ。上流のみ変更 115 件採用 / wanonwan 独自部品維持 / 衝突 33 件個別マージ / 汎用新規部品のみ採用 |
| ステータス | ✅ 検証完了 |
| 前提 Plan | なし |
| PR | [#97](https://github.com/sasakiyusuke2017015/wanonwan/pull/97) |
...
2026-07-15 時点）を、wanonwan の vendored コピー `packages/ui`（`@ui-catalog/core`）へ
**選択的マージ**で取り込む。wanonwan 側で独自に育てた部品・修正を失わずに、
...
- `package.json`: `@ui-catalog/core` 名義・wanonwan 独自 exports を維持したまま、
...
- **zip での全面上書き**（wanonwan 独自部品 123 ファイルが消えるため不可）
- **DataTable / Toast の上流版採用**（wanonwan 版が admin 画面・ToastProvider の中核。
  wanonwan 版を優先し、上流版の差分は取り込まない）
...
- `packages/ui` は Phase 0（`e48d620`）で vendoring 後、wanonwan 側だけで 20 コミット以上
...
| 上流のみ変更 | 115 | 上流版を採用 |
| wanonwan のみ変更 | 16 | 現状維持 |
| 両方変更（衝突） | 23 + DataTable 系 10 | 個別マージ |
| wanonwan 独自部品（上流に無い） | 123 | 必ず維持 |
| 上流の新規部品 | 248 | 汎用のみ採用 |

- 衝突の性質:
  - **Icon / icons.ts**: 上流が lucide-react ベースへ全面書き換え（+350 行）。
    wanonwan 追加の 5 アイコン（plus / pencil / copy / ban / grip）は上流レジストリに
    存在する見込み → マージ時に全数検証
  - **Modal / Dialog**: wanonwan の focus-trap / ARIA 修正が上流にも入り、ほぼ収束済み（差分 30〜40 行）
  - **DataTable**: 双方が別進化（実質差分 170 行）→ wanonwan 版優先
  - **Toast**: 双方が独立に追加 → wanonwan 版優先（ToastProvider / useToast と結合済み）
  - **tokens.css / globals.css**: 上流がトークン大幅追加（+123 行）、wanonwan 変更は 9 行 → 機械的マージ可
...
2. **Phase 1 — 無衝突置換**: 上流のみ変更の 115 ファイルを上流版で置換
   （改行コードは現行リポジトリに合わせる）。wanonwan のみ変更 16 + 独自部品 123 は触らない
3. **Phase 2 — 衝突マージ**:
   - Icon / icons.ts: 上流版採用 + wanonwan 追加アイコン 5 種の存在検証。
     **`ban` は上流 registry に無いことを計画レビューで確認済み → wanonwan 定義を再追補する**。
     残り 4 種（plus / pencil / copy / grip）は上流に存在
   - Modal / Dialog / MarkdownPreview / EventModal / AgendaView / Input / Select /
     IconButton: 上流版ベースに wanonwan 差分を再適用
   - DataTable 系 10 ファイル + Toast: wanonwan 版維持（上流差分は不採用）
   - tokens.css / globals.css: 上流トークン追加 + wanonwan の 9 行を保持
...
| リスク | 影響 | 緩和策 |
|---|---|---|
| Icon lucide 化で wanonwan 使用アイコンが欠落・見た目変化 | 全画面 | マージ時に使用アイコン名を全数照合 + 目視確認 |
| 上流の破壊的 API 変更（Badge / Tooltip / IconButton 等） | apps/web 119 import | typecheck + build で検出。壊れたら該当部品のみ wanonwan 版へ戻す |
| 見送り部品への内部参照が採用部品に残る | ビルド失敗 | typecheck で検出し、参照元ごと採否を再判断 |
| tokens.css 追加による既存テーマ（rose/sharp/fabric）の見た目崩れ | 全画面 | 目視確認。崩れたら wanonwan トークンを優先 |
...
| 日時 | 判断 | 理由 |
|---|---|---|
| 2026-07-16 | 全面上書きではなく選択的マージ | wanonwan 独自部品 123 ファイルと独自修正 16 ファイルが消えるため |
| 2026-07-16 | DataTable / Toast は wanonwan 版優先 | admin 画面・ToastProvider の中核で、上流版とは別進化。置換リスクが利益を上回る |
| 2026-07-16 | 新規部品は汎用のみ採用 | ai-education ドメイン専用（クイズ / 試験系）は wanonwan で使途がなく dead code になる（evergreen 方針） |
| 2026-07-16 | package.json 名義 / exports は wanonwan 版維持 | apps/web の import 119 箇所と wanonwan 独自部品の exports を壊さないため |
| 2026-07-17 | packages/ui の vitest は今回も CI ゲート外のまま（手元実行で担保） | `test` script 追加は破損スイート除外の経緯（CI 除外）と絡むため本 Plan のスコープ外。要否は別途判断 |
| 2026-07-17 | `ban` アイコンは wanonwan 独自 SVG を維持（実装時の訂正: 上流 lucide-registry にも `Ban` は存在した） | Icon は独自 SVG を registry より優先する設計のため、独自定義維持がそのまま有効 |
...
| 2026-07-17 | VERSION_REGISTRY / versions.json に wanonwan 独自部品 20 件を復帰（コードレビュー指摘） | 上流 JSON 全面置換で欠落していた。version 追跡メタは「現存 export 部品と一致」を方針とする |
...
- [x] Phase 2: 衝突マージ（33 ファイル。DataTable / Toast は wanonwan 版維持）
...
- [x] Phase 5: 検証（`origin/develop` へ rebase 後、新ツールチェーンで再実行）
  ...
  - [x] `pnpm -r typecheck`
  - [x] `pnpm --filter @wanonwan/web build`
...
- [x] PR 作成・レビュー（[#97](https://github.com/sasakiyusuke2017015/wanonwan/pull/97) merge 済み）
