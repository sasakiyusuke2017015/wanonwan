...
- [ ] 1. settings.json の allow 全削除
- [ ] 2. hooks の全削除 + docs 更新
- [ ] 3. skills / MCP テンプレの整理（2 の後）
- [ ] 4. `gen-outputs-readme --check` + CI 配線
- [ ] マージ後検証
  - [ ] permission 3 層に `git push` / `gh pr` / `git commit` が無く、実セッションで push が確認を求められる
  - [ ] `outputs/plans/**` の Write がブロックされない
  - [ ] 削除した skill / MCP への参照が残っていない
  - [ ] step 4 の PR 上で、README をずらすと CI が落ち、再生成すると緑になる
